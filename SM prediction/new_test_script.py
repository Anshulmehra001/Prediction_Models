...
def generate_signal(close, sma, rsi):
...
def predict_next_day_close(last_close, sma_20, ema_10, rsi_14):
    ...
    if ema_10 > sma_20 and last_close > ema_10:
        bias += 0.005
    elif ema_10 < sma_20 and last_close < ema_10:
        bias -= 0.003

    bias += 0.005 * (2 * np.random.random() - 1)
    return round(last_close * (1 + bias), 2)

def run():
    print(f"Fetching data for {STOCK_TICKER}…")
    end_date = datetime.date.today()
    start_date = end_date - datetime.timedelta(days=HISTORY_DAYS + 30)
    df = yf.download(STOCK_TICKER, start=start_date, end=end_date)
    if df.empty:
        print("Error: no data.")
        return
    ...
    ema = float(latest['EMA_10'])
    rsi = float(latest['RSI_14'])

    signal = generate_signal(close, sma, rsi)
    pred = predict_next_day_close(close, sma, ema, rsi)
    pct = (pred - close) / close * 100
    arrow = "▲" if pct >= 0 else "▼"
    ...
    print(f"\n📊 Signal: {signal}")
    print(f"Predicted Next Close: ₹{pred:.2f} ({pct:.2f}% {arrow})")
